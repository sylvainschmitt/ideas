...
This chapter identify an increase in Januaryevapotranspiration in Guiavare on deforested (+1mm/8day \[1-1\]) an degraded (+0.5mm/8day \[0.5-1\]) lands compared to forest that remained undisturbed. However, due to the spatial structure of the deforestation arc, this result is not robust to spatial autocorrelation, with only non significant marked effect of deforested forest on evapotranspiration increase (+2mm/8day \[-3-7\]). Further directions include (1) using other territories for statistical robustness, and (2) using finer temporal indices such as extreme events.
...
We used total evapotranspiration (ET) data from MOD16A2GF version 6.1 MODIS ET data downscaled at 0.01-degree resolution grid (originally 500-m). We kept only pixels with (1) only good quality (MODLAND), (2) on both Terra and Aqua sensors,(3) detectors fine for up to 50% of channels 1, 2, (4) without significant clouds, (5) and the main method used for best result possible (no saturation). Indeed, extensive cloud cover can reduce the spatial and temporal availability of satellite data. For this reason, we focus our analysis on the dry season when there is less cloud cover. We thus worked with January the driest month in Guiavare. Dry season evapotranspiration changes were then calculated by comparing evapotranspiration of the driest month for two periods at the end (2015 to 2020) and start (2001 to 2006) of the study period. Using 5-y averages reduces the influences of climate variability. We computed both variations in mean evapotranspiration (et), maximum evapotranspiration (etmax), and minimum evapotranspiration (etmin). As a final preprocessing step, we used forest change classification to keep only pixels undisturbed, degraded or deforested in 2020. Our final dataset comprised n=\~74 thousands pixels (1 km^2^ in extent) over the Guiavare region.

```{r etplot}
area <- st_read("data/guaviare/guaviare.shp", quiet=T)
temp_r <- rast("results/data/modis_et_guaviare.nc")
ggplot() +
  tidyterra::geom_spatraster(data = temp_r[["et"]]) +
  theme_bw() +
  scale_fill_viridis_c(expression(delta~ET)) +
  geom_sf(data = area, fill = NA, linewidth = 2) +
  ggtitle("Guiavare January mean evaportanspiration variation", 
...
The ANOVA and Tukey HSD tests revealed significantly higher evapotranspiration in deforested (+0.05°C \[0.04-0.06\]) an degraded (+0.03 \[0.02-0.04\]) lands compared to forest that remained undisturbed. But the autocorrelogram revealed significant positive spatial autocorrelation up to 100-km.

```{r anovaplot}
data %>% 
  gather(variable, value, -x, -y, -Dec2020, -type) %>% 
  mutate(varlong = recode(variable, "et" = "mean", 
                          "etmax" = "maximum", "etmin" = "minimum")) %>% 
  ...
  facet_wrap(~ varlong) +
  scale_fill_discrete("") +
  ggtitle("Guiavare January evapotranspiration variation per forest change")
```
...
data %>% 
  ...
  split(.$varlong) %>% 
  lapply(function(tab)
    TukeyHSD(aov(value ~ type, tab))$type %>% 
                           as.data.frame() %>% 
                           rownames_to_column("comp")) %>% 
  bind_rows(.id = "varlong") %>% 
  ...
  facet_wrap(~ varlong, scales = "free_x") +
  ggtitle("Tukey honest significant difference tests")
```

```{r anovacor}
data$res_tas <- residuals(aov(et ~ type, data))
...
The LRSRE revealed only non significant marked effect of deforested forest on evapotranspiration increase (+2mm/8day \[-3-7\]).

```{r spammfit}
#| include: false
#| eval: false
m <- spaMM::fitme(et ~ type + Matern(1|y+x), data2)
tab <- summary(m)$beta_table
save(m, tab, file = "save/evapotranspiration.Rdata")
```

```{r spammplot}
load("save/evapotranspiration.Rdata")
as.data.frame(tab) %>% 
