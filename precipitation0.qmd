...
ggplot() +
  ...
  ggtitle("Guiavare January Precipitation anomaly", 
...
precip_r <- rast("results/data/chirps_guaviare.nc")
data.frame(values(precip_r$`precipitation_month=1`), values(forest_r)) %>% 
  rename(precipitation = "precipitation_month.1") %>% 
  filter(loss < 0.5) %>% 
  na.omit() %>% 
  mutate(loss_class = cut(loss, breaks = seq(0, 1, by = 0.1))) %>% 
  na.omit() %>% 
  ggplot() +
  geom_boxplot(aes(loss_class, precipitation)) +
  theme_bw() +
  ggpubr::stat_anova_test(aes(loss_class, precipitation))
```
...
```{r}
TukeyHSD(aov(`precipitation_month=1` ~ type, data))$type %>% 
  knitr::kable()
```

```{r}
data$res_tas <- residuals(aov(`precipitation_month=1` ~ type, data))
...
m <- lm(`precipitation_month=1`  ~ type + y + mem1 + mem2 + mem3, data2) 
```
...
```{r}
m <- spaMM::fitme(`precipitation_month=1` ~ type + Matern(1|y+x), data2)
summary(m)
```
